...
class TestRefreshIntervals:
    ...
    async def test_overrides_apply(self):
        ...
        with patch("backend.scanner.engine.get_config_override", new=AsyncMock(return_value=override)):
            await engine._refresh_intervals()
    ...
    async def test_invalid_json_leaves_current_values(self):
        ...
        with patch("backend.scanner.engine.get_config_override", new=AsyncMock(return_value="not-json")):
            await engine._refresh_intervals()
